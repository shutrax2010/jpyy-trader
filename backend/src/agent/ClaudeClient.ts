...
function isApiKeyConfigured(): boolean {
  return !!config.ANTHROPIC_API_KEY && !config.ANTHROPIC_API_KEY.startsWith('sk-ant-...');
}

export class ClaudeClient {
  static async decide(state: MarketState, mode: AgentMode): Promise<Decision> {
    if (mode === 'random') return ClaudeClient._decideRandom(state);
    if (isApiKeyConfigured()) {
      try {
        return await ClaudeClient._decideWithApi(state, mode);
      } catch (e) {
        console.error('[ClaudeClient] API error, falling back to dummy:', e instanceof Error ? e.message : e);
      }
    }
    return ClaudeClient._decideDummy(state, mode);
  }
